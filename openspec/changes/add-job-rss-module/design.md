...
10. **Deploy setup via activation script** — `system.activationScripts.jobRssDeploy` runs Laravel's `migrate --force` (idempotent), `passport:keys` (no-op if keys exist), and `passport:client --client` (guarded by a sqlite3 check for duplicate clients) on every `nixos-rebuild switch`. After passport setup, it checks if `onlinejobsph_job_listings` and `indeed_job_listings` tables are both empty — if so, dispatches `app:scrape` to the database queue. All artisan commands use `php -d variables_order=EGPCS` to ensure `$_ENV` is populated for Laravel's `env()` helper (Laravel 11 only reads `$_ENV`, not `getenv()`). A final `chown` fixes ownership of files created as root.

11. **Dedicated queue worker daemon** — `systemd.services.${app}-queue-worker` runs `php artisan queue:work --sleep=3 --tries=3` as the `jobs-jobs` user, processing queued jobs (scraping, description fetching) continuously. Same env vars as the PHP-FPM pool plus `QUEUE_CONNECTION=database`. Starts at boot via `multi-user.target` with `Restart=always`. Uses `-d variables_order=EGPCS` for the same reason as the activation script.
