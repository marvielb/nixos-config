...
- External flake modules (disko, preservation) are imported inside the shared helpers (`_disko.nix` and `modules/preservation.nix`) respectively, making each self-contained. The disko helper reads `custom.disko` (device + swap size) which the host sets inline.
