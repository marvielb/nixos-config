...
modules/
  ...
  hardware/
    audio.nix                 # flake.modules.nixos.hardware_audio (pipewire)
    qemu-vm.nix               # flake.modules.nixos.hardware_qemu
  profiles/
    desktop.nix               # flake.modules.nixos.profile_desktop (desktop composite)
  security/
  ...
  hosts/
    ...
    _disko.nix                # shared parameterized disko layout (private helper via custom.disko)
    <hostname>/
      default.nix             # host picks from catalog + HM user block + custom.disko/custom.persist
      _hardware.nix           # hardware config (generated by nixos-anywhere)
  preservation.nix            # flake.modules.nixos.preservation — persistence collector
```
...
- External flake modules (disko, preservation) are imported inside the shared helpers (`_disko.nix` and `modules/preservation.nix`) respectively, making each self-contained. The disko helper reads `custom.disko` (device + swap size in MiB + optional `encrypt` bool) which the host sets inline. When `encrypt = true`, the root partition becomes LUKS2 (`cryptroot`, interactive passphrase, no hibernation) and swap is a btrfs swapfile inside the container.
- Host-specific config (users, timezone, ssh, firewall, `custom.persist`, `custom.disko`) goes inline in the NixOS module body.
- Private helpers (`_`-prefixed files like `_hardware.nix`, `_disko.nix`) are imported via relative path. They're plain NixOS modules, not flake-parts modules, so import-tree skips them.
...
| `flake.modules.nixos.profile_<name>` | Composite feature — bundles other catalog entries (e.g. `profile_desktop`) |
| `custom.persist` in feature module | Declare persistence where the feature lives, not in the host collector |
| `custom.persist.home.directories`  | Per-user paths merged into every user by the preservation collector |
...
The preservation catalog entry (`modules/preservation.nix`) acts as a single
**collector**, mapping the merged `config.custom.persist` into
...
Hosts just import `preservation` from the catalog and inline their per-host
`custom.persist` block (host-level dirs like `/etc/ssh`, `/var/log`, machine-id,
and per-user `.ssh`).
